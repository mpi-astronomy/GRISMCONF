...
EXAMPLE_NIRISS_CONF_URL = "https://raw.githubusercontent.com/npirzkal/GRISM_NIRISS/refs/heads/master/V3/NIRISS_F150W_GR150C.conf"
...
def collect_test_data(keep=False):
    """Collects test data for the GrismConf class."""
    fname = fetch_file(EXAMPLE_NIRISS_CONF_URL)
    temp_files = [fname]

    # Fetch dependencies
    temp_files += fetch_conf_dependencies(EXAMPLE_NIRISS_CONF_URL, fname)
...
def test_example1(collect_test_data):
    ...
    assert dx01.shape == (2,)
    assert np.isclose(dx01[0], 63.55433723, atol=1e-6), (
        "dx01[0] should be close to 63.55433723"
    )
    assert np.isclose(dx01[1], -268.30487088, atol=1e-6), (
        "dx01[1] should be close to  -268.30487088"
    )
