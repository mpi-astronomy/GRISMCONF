try:
    ...
    from grismconf.jax import JaxGrismConf as GrismConf
    ...
    def test_example1(collect_test_data):
        ...
        dx01 = C.DISPX("+1", x0, y0, jnp.array([0, 1], dtype=jnp.float32))
        assert dx01.shape == (2,)
        assert dx01[0] < dx01[1], "dx01 should be increasing"
        assert jnp.isclose(dx01[0], -49.85361111, atol=1e-6), (
            "dx01[0] should be close to -49.85361111"
        )
        assert jnp.isclose(dx01[1], -48.48791052, atol=1e-6), (
            "dx01[1] should be close to  -48.48791052"
        )

        # Get a list of all dxs value along this trace
        dxs = jnp.arange(dx01[0], dx01[1])
        ...
        assert jnp.all(ts >= 0) and jnp.all(ts <= 1), "ts should be in the range [0, 1]"
        assert jnp.isclose(ts[0], 0.0, atol=1e-6), "ts[0] should be close to 0.0"
        assert jnp.isclose(ts[-1], 0.72035807, atol=1e-6), (
        ...
        assert jnp.all(dys <= 0), "dys should be non-positive"
        assert jnp.isclose(dys[0], -1530.876494, atol=1e-6), (
            "dys[0] should be close to -1530.876494"
        )
        assert jnp.isclose(dys[-1], -1373.38785167, atol=1e-6), (
        ...
        assert jnp.all(wavs >= 2.4) and jnp.all(wavs <= 2.7), (
            "wavs should be in the range [2.4, 2.7]"
        )
        assert jnp.isclose(wavs[0], 2.49696493, atol=1e-6), (
            "wavs[0] should be close to 2.49696493"
        )
        assert jnp.isclose(wavs[-1], 2.64881764, atol=1e-6), (
    ...
    def test_example2(collect_test_data):
        ...
        assert jnp.isclose(ratio_12, -0.523209215269946, atol=1e-14), (
